...
public class IProductServiceImp implements IProductService
{
    ...
    public List<Product> viewAllProducts() throws ProductIdException {
		...
		if (productlist.isEmpty())
		   throw new ProductIdException(ShoppingConstants.PRODUCT_NOT_FOUND);
	...
	public Product addProduct(ProductDto productDto) throws ValidateProductException, CategoryNotFoundException{
		...
		Optional<Category> optCategory = categoryDao.findById(productDto.getCategoryId());
		if (!optCategory.isPresent())
			throw new CategoryNotFoundException(ShoppingConstants.CATEGORY_NOT_FOUND);
	...
	@Override
	public List<Product> viewProductsByCategory(Integer categoryId) throws CategoryNotFoundException{
		List<Product> productlist = productDao.findAll();
		Optional<Category> optCategory = categoryDao.findById(categoryId);
		if (!optCategory.isPresent())
			throw new CategoryNotFoundException(ShoppingConstants.CATEGORY_NOT_FOUND);
		productlist.stream()
		.filter(p ->p.getCategory().getCatId().equals(categoryId))
